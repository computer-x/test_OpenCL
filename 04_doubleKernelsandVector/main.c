...
int main()
{
    ...
    //cl_int dtimes = 3;
    //cl_int dbias = 10; //不能再用这种方法了

    
    cl_int htimes[8] = {3, 3, 3, 3, 3, 3, 3, 3}; //向量的话，就不能用参数传递了，否则每行只有前两个数据有计算结果，我猜测是不是参数空间内存限制的问题？
    cl_int hbias[8] = {10, 10, 10, 10, 10, 10, 10, 10};
    cl_mem dtimes = clCreateBuffer(ctx->context, CL_MEM_READ_ONLY, 8 * sizeof(float), NULL, NULL);
    cl_mem dbias = clCreateBuffer(ctx->context, CL_MEM_READ_ONLY, 8 * sizeof(float), NULL, NULL);
    ...
    err = clEnqueueWriteBuffer(ctx->queue, dtimes, CL_TRUE, 0, 8 * sizeof(int), htimes, 0, NULL, NULL);
    err = clEnqueueWriteBuffer(ctx->queue, dbias, CL_TRUE, 0, 8 * sizeof(int), hbias, 0, NULL, NULL);
    
    err = clEnqueueWriteBuffer(ctx->queue, ctx->dsrc, CL_TRUE, 0, 4 * 8 * sizeof(int), hsrc, 0, NULL, NULL);
    
    size_t global[2] = { 4 };
    size_t local[2] = { 4 };
    err = clEnqueueNDRangeKernel(ctx->queue, ctx->kernel[1], 1, NULL, global, local, 0, NULL, NULL);
